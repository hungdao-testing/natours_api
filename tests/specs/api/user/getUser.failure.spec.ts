...
testPW.describe.only('Get Users', () => {
    testPW('is failed because of guide-user permission', async ({ request, authenBy }) => {
        ...
        expect(assertFailedBy('You do not have permission to perform this action', users)).toBeTruthy()
    })
    ...
    testPW(
        ...
        async ({ request, authenBy }) => {
            ...
            const users = await getUser(request, token, nonExistingUserId)
            expect(users.statusCode).toBe(404)
            expect(assertFailedBy('No users found with the passing ID', users)).toBeTruthy()
        },
    )
    testPW(
        'is failed because the request is calling to inactive user',
        async ({ request, authenBy }) => {
            const token = await authenBy('ADMIN')
            const inactivateUser = getTestUserByRole('USER', false)
            const users = await getUser(request, token, inactivateUser._id)
            expect(users.statusCode).toBe(404)
            expect(assertFailedBy('No users found with the passing ID', users)).toBeTruthy()
        },
    )
